...
const PLANS = [
    { key: 'starter', name: 'Starter', price: '$49/mo', roles: '5 roles/month', icon: Zap, color: 'var(--color-text-muted)', features: ['AI-generated assessments', 'Candidate ranking', 'Email notifications', 'Score breakdown'] },
    { key: 'growth', name: 'Growth', price: '$149/mo', roles: '20 roles/month', icon: TrendingUp, color: 'var(--color-primary)', features: ['Everything in Starter', 'Priority AI scoring', 'Advanced analytics', 'Custom email templates', 'Webhook notifications'] },
    { key: 'enterprise', name: 'Enterprise', price: 'Custom', roles: 'Unlimited roles', icon: Building2, color: 'var(--color-text-subtle)', features: ['Everything in Growth', 'Dedicated support', 'Custom AI prompts', 'SSO integration', 'SLA guarantee'] },
];
    ...
    return (
        <div style={{ padding: 32, maxWidth: 960 }} className="animate-fadeInUp">
            ...
            <p style={{ color: 'var(--color-text-muted)', marginBottom: 28, fontSize: 14 }}>
                Current plan: <span style={{ color: 'var(--color-primary)', fontWeight: 600, textTransform: 'capitalize' }}>{currentPlan}</span>
                    ...
                    return (
                        <div key={plan.key} className="card" style={{ border: active ? `2px solid ${plan.color}` : '1px solid var(--color-border)', position: 'relative', display: 'flex', flexDirection: 'column', gap: 16 }}>
                            {active && <div style={{ position: 'absolute', top: 12, right: 12, fontSize: 11, background: plan.color, color: 'white', padding: '2px 10px', borderRadius: 20, fontWeight: 600 }}>Current</div>}
                             <div style={{ width: 40, height: 40, background: 'var(--color-bg-tertiary)', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
